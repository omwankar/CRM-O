...
import type { EnquiryStage, Quotation, QuotationStatus } from '@/types/quotations';
import { ENQUIRY_STAGE_LABELS } from '@/types/quotations';
...
function finalStatusLabel(stage: EnquiryStage, status: QuotationStatus, outcome?: string | null): string {
  if (status === 'approved') {
    return stage === 'won_lost_closed' ? 'Won' : `Won (${ENQUIRY_STAGE_LABELS[stage]})`;
  }
  if (status === 'rejected') {
    return stage === 'won_lost_closed' ? 'Lost' : `Lost (${ENQUIRY_STAGE_LABELS[stage]})`;
  }
  if (status === 'cancelled') {
    return stage === 'won_lost_closed' ? 'Closed' : `Closed (${ENQUIRY_STAGE_LABELS[stage]})`;
  }

  if (stage === 'won_lost_closed') {
    if (outcome?.trim()) return 'Won / Lost / Closed';
    return 'Won / Lost / Closed — pick outcome on the tracker';
  }
...
export function QuotationStatusBar({ quotation, enquiryStage }: Props) {
  const updatedBy =
